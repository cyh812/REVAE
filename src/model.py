...
class REVAE_V64(nn.Module):
    def __init__(
        ...
        # -----------------------
        self.dec_fc = nn.Sequential(
            nn.Linear(self.z_total_dim, 128 * 8 * 8),
            nn.ReLU(inplace=True),
        )
...
class REVAE_V128(nn.Module):
    def __init__(
        ...
        # -----------------------
        self.dec_fc = nn.Sequential(
            nn.Linear(self.z_total_dim, 256 * 8 * 8),
            nn.ReLU(inplace=True),
        )
    ...
    def decode(self, z_c, z_s, z_n):
        z = z_c + z_s + z_n
        h = self.dec_fc(z).view(-1, 256, 8, 8)
        x_logits = self.decoder(h)  # (B,3,224,224)
        return x_logits
